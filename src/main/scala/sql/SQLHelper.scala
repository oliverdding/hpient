...
import Utils.{dateFmt, dateTimeFmt}
...
trait SQLHelper {
  ...
  private[sql] def quoted(token: String) = s"`$token`"

  private def escapeSql(value: String): String =
    StringUtils.replace(value, "'", "''")

  private def compileValue(value: Any)(implicit tz: ZoneId): Any = value match {
    case stringValue: String => s"'${escapeSql(stringValue)}'"
    case timestampValue: Timestamp => "'" + timestampValue + "'"
    case timestampValue: Instant =>
      s"'${dateTimeFmt.withZone(tz).format(timestampValue)}'"
  ...
  private def compileFilter(f: Filter)(implicit tz: ZoneId): Option[String] =
    Option(f match {
      case AlwaysTrue => "1=1"
      ...
      case GreaterThan(attr, value) =>
        s"${quoted(attr)} > ${compileValue(value)}"
      ...
      case In(attr, value) if value.isEmpty =>
        s"CASE WHEN ${quoted(attr)} IS NULL THEN NULL ELSE FALSE END"
